...
// Placeholder reviews - TO BE REPLACED WITH REAL GBP REVIEWS
// These follow the exact structure needed. Replace with actual review content from Google Business Profile
...
// Helper to get featured reviews (for homepage carousel)
export function getFeaturedReviews(count: number = 5): Review[] {
    return reviews.slice(0, count);
}

// Helper to get reviews by city
export function getReviewsByCity(city: string): Review[] {
    return reviews.filter(r => r.city && r.city.toLowerCase() === city.toLowerCase());
}
